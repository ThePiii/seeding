...
logger = Logger(f'logs/mmbt_base_freeze.log', resume=True)
...
class ImageEncoder(nn.Module):
    def __init__(self):
        ...
        self.pool = nn.AdaptiveAvgPool2d(POOLING_BREAKDOWN[2])
...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = url_to_pilimg_train(self.data.iloc[0]['trendImageUrl'].split(',')[0])
...
def evaluate(model, eval_dataset, criterion, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):  # tqdm用于可视化展示进度
        ...
        with torch.no_grad():
            ...
            logits = outputs   # outputs只输出logits了
...
def train(train_dataset, evaluate_dataset, model, criterion, EPOCHS, lr):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs = model(**inputs)
            logits = outputs   # 简化了，outputs只返回logits
...
def main():
    ...
    model = MMBTClassfier(mmbt_config, transformer, img_encoder)
    model = model.to('cuda')
