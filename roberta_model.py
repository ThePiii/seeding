...
class BertClassifier(nn.Module):
    def __init__(self, n_classes):
        ...
        self.bert = BertModel.from_pretrained("hfl/chinese-roberta-wwm-ext",
                                              return_dict=False,    # 不加return_dict=False的话，pooled_output返回的是str
                                              cache_dir='cache',
                                              output_hidden_states=True)  # 获取隐藏层输出，用first-last-avg
    ...
    def forward(self, input_ids, attention_mask):
        # with torch.no_grad():
        # 微调Bert
        seq, pooled_output, hidden = self.bert(
            input_ids=input_ids,
    ...
    if train:
        ...
        for epoch in range(EPOCHS):
            ...
            if train_auc > best_auc:
                torch.save(model.state_dict(), f'./model/roberta.pth')
                best_auc = train_auc
            ...
            df_test = pd.read_csv('./data/test_new.csv', lineterminator='\n').sample(50)
