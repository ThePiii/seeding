...
class bert_dataset(Dataset):
    def __init__(self, contents, targets, tokenizer, max_len=320):
...
if __name__ == '__main__':
    ...
    tokenizer = BertTokenizerFast.from_pretrained('bert-base-chinese', cache_dir='cache')
    batch_size = 8
    max_len = 256
    EPOCHS = 10
    ...
    if test:
        for epoch in range(EPOCHS):
            model = BertClassifier(2).to(device)
            model.load_state_dict(torch.load(f'./model/Bert-base/bert_base_{epoch}.pth'))
